...
class ExerciseRepository @Inject constructor(
    ...
    // -- Lazy Loading: Download exercise image on-demand (when user clicks) -------
    // This is the ONLY place that downloads images after the initial sync.
    // Returns true if image was successfully downloaded, false if already cached or failed.
    suspend fun downloadExerciseImage(exerciseId: String): Result<Boolean> =
        runCatching {
            // Check if already cached
            if (cachedExerciseDao.hasImageCached(exerciseId)) {
                return@runCatching false
            }

            // Fetch exercise metadata to get remote image URL
            val exercise = cachedExerciseDao.getExerciseById(exerciseId)
                ?: throw NoSuchElementException("Exercise $exerciseId not found")

            // If no remote URL stored, skip download (happens with built-in exercises)
            if (exercise.gifUrl.isBlank()) {
                return@runCatching false
            }

            // Download and save locally — imageDownloader returns "file://..." or ""
            val localPath = imageDownloader.download(exerciseId.toIntOrNull() ?: 0, exercise.gifUrl)

            if (localPath.isNotBlank()) {
                // Mark as downloaded in DB
                cachedExerciseDao.markImageDownloaded(exerciseId, localPath)
                return@runCatching true
            }
            false
        }

    // -- Wger sync (initial bulk import) ----------------------------------------
    // Downloads metadata for every Portuguese-translated exercise from Wger.
    // Images are NOT downloaded here — they're fetched lazily when user clicks.
    // This keeps initial sync fast and respects API rate limits.
    //
    ...
    suspend fun syncAllExercisesFromWger(onProgress: (Int, Int) -> Unit = { _, _ -> }): Result<Int> =
        runCatching {
            ...
            while (true) {
                ...
                // NEW: Don't download images during sync — save metadata with empty gifUrl
                val batch = page.results.mapNotNull { dto ->
                    dto.toDomain(ptLanguageId, localImagePath = "")  // No image download here
                }
                if (batch.isNotEmpty()) {
                    // Save with hasImage = false (images will be downloaded on-demand)
...
private fun Exercise.toCachedEntity(cachedAt: LocalDateTime) = CachedExerciseEntity(
    ...
    hasImage = gifUrl.isNotBlank()  // Mark as cached if already has a local path
)
